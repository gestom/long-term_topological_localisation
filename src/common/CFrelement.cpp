...
{
	timePeriod = 24*3600;  //consider maximal periodicity as one week by default
...
CFrelement::CFrelement(int orderi,float gaini,int length)
{
	gain = gaini;
	order = orderi;
	signalLength = length;
	outliers = 0;
	timePeriod = 7*24*3600;  //consider maximal periodicity as one week by default
...
/*void CFrelement::add(unsigned int time,unsigned char value)
{
...
float CFrelement::evaluate(unsigned int* times,unsigned char* signal,int length,int orderi)
{
	float error = 0;
	int samples = 0; 
	int myOrder = order;
	...
	{	
		error += fabs(estimate(times[i],order)-signal[i]);
		samples++;
	}
	order = myOrder;
...
void CFrelement::build(unsigned int* times,unsigned char* signal,int length,int orderi)
{
	if (orderi > MAX_ADAPTIVE_ORDER+1){
		build(times,signal,length,MAX_ADAPTIVE_ORDER);
		if (debug) printf("Order:");
		float minError = 100000000;
		int minIndex = 0;
		for (int i = 0;i<MAX_ADAPTIVE_ORDER;i++)
		{
			float error = evaluate(times,signal,length,i);
			if (error < minError){
				minError = error;
				minIndex = i;
			}
		...
		if (debug) printf(" - %i \n",minIndex);
		build(times,signal,length,minIndex);
	}else{
...
float CFrelement::estimate(int time,int maxOrder)
{
	float estimate = gain;
...
/*retrieves a boolean*/
unsigned char CFrelement::retrieve(unsigned int timeStamp)
{
	int i = 0;
	for (i= 0;i<outliers;i++){
		if (timeStamp < outlierSet[i]) break;
	}
	return (estimate(timeStamp,order) >= 0.5)^(i%2);
}
...
int CFrelement::save(char* name,bool lossy)
{
	...
	if (file == NULL)
	{
		fprintf(stderr,"Could not save frelement to %s\n",name);
		return -1;
	}
